...
const Navbar = () => {
  ...
  const navItems = [
    ...
    { path: '/appointment', label: 'Book Appointment' }, // 🔄 Changed for clarity
  ];

  return (
    <div className="w-full">
      {/* 🔴 Top Strip */}
      <div className="bg-red-600 h-11 flex justify-end items-center px-6 gap-6 text-white text-sm font-medium">
      ...
      <nav className="bg-white shadow-sm">
        <div className="container mx-auto px-4 py-4 flex items-center justify-between">
          ...
          {/* Desktop Navigation */}
          <ul className="hidden md:flex gap-10 text-gray-800 font-medium text-base md:text-lg">
            {navItems.map((item) => (
        ...
        {/* Mobile Navigation */}
        {menuOpen && (
          <ul className="md:hidden px-4 pb-4 flex flex-col gap-4 text-gray-800 font-medium text-base bg-white">
            {navItems.map((item) => (
              <li key={item.path}>
                <NavLink
                  to={item.path}
